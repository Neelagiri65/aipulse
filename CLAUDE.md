...
- **LLM:** ONE declared live-path caller since 2026-09-26 (founder decision, AUDITOR-REVIEW: PENDING): `src/lib/summaries/machine-summary.ts` writes a ≤2-sentence **machine summary** of the article a Hacker News link post points to, only when the source published no text, in the `wire-ingest-hn` cron (never per view). Model: NVIDIA NIM `openai/gpt-oss-20b` (chosen by measurement, `scripts/machine-summary-smoke.ts`; reasoning effort low). Gated: off unless `MACHINE_SUMMARIES=on` AND `NVIDIA_NIM_KEY` are set on Vercel; ≤5 per run, ≤60 per UTC day; grounded or dropped (every number and capitalised name must appear in the article); stored with model, prompt version, input URL + hash; carried as `Card.machineSummary` (never `summary`) and always shown labelled as machine-written. Registry: `machine-summary-nim`; `src/app/__tests__/audit-llm-claim-is-absolute.test.ts` pins it as the only caller and keeps the `/audit` engine free of it (its "no LLM calls" copy describes the audit engine and stays true). Scripts outside `src/`: `scripts/video/curate-stories.ts --llm` and `scripts/video/generate-script.ts` (NIM or Gemini, manual) and `scripts/video/generate-narration.ts` (local Ollama); the daily video ships template editorials. If a deep scan is ever built, it stays opt-in with the user's own key. Update this line when any of these facts change.
...
- `NVIDIA_NIM_KEY` — read by `src/lib/summaries/run-hn.ts` (`machineSummaryConfig`, via the `wire-ingest-hn` route, Vercel env; only with `MACHINE_SUMMARIES=on`) and by `scripts/video/curate-stories.ts --llm` / `scripts/video/generate-script.ts`. Server-side only, never bundled.
