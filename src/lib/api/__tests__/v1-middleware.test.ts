...
describe("v1-middleware", () => {
  ...
  describe("handleV1Request", () => {
    ...
    // A product-name sweep renamed these once (#108 → `X-gawk.dev-Generated-At`) and nothing
    // failed, because no test asserted the shape of a header NAME. This one does: the names are
    // a contract other people's code reads, and a dot in a header name trips naive parsers.
    it("spells every Gawk header as X-Gawk-*, with no dot in the name", async () => {
      const res = await handleV1Request(makeRequest(), async () => ({
        body: { ok: true },
        meta: { generatedAt: "2026-09-08T00:00:00.000Z", sourceCount: 42, cacheMaxAge: 300 },
      }));

      const names = [...res.headers.keys()].filter((n) => n.toLowerCase().startsWith("x-gawk"));
      expect(names.length).toBeGreaterThanOrEqual(3);
      for (const name of names) {
        expect(name).toMatch(/^x-gawk-[a-z-]+$/i);
