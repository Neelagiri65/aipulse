...
export async function sendConfirm(input: SendInput): Promise<SendResult> {
  ...
  const from = process.env.EMAIL_FROM_ADDRESS ?? "gawk.dev <digest@example.invalid>";
  return sender.send({
    from,
    to: input.to,
    subject: "Confirm your gawk.dev subscription",
    react: ConfirmEmail({
...
export async function sendUnsubscribeReceipt(
  ...
  return sender.send({
    from,
    to: input.to,
    subject: "You have unsubscribed from gawk.dev",
