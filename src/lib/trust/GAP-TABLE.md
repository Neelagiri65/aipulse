...
| benchmarks (Elo) | ✗ | ✓ | ✓ | · | · | ✓ | benchmarks-trust + panel-sources-trust | delta-provenance done. A/V at CONSUMER surfaces: digest section ✓ (digest-trust); panel citation now READS the registry entry (no hardcode drift) + pinned resolvable (panel-sources-trust). F: LMArena publishes irregularly — a window would be invented |
...
| agents | ✗ | ✓ | ✓ | · | · | ✓ | agents-trust + panel-sources-trust | delta-provenance (bootstrap/new-from-zero) framed via invariant. A/V at CONSUMER surfaces: digest section ✓ (digest-trust); panel URL construction pinned (every registry githubRepo = bare owner/repo, constructed link resolvable — the #53 class can't be curated in) |
