...
const CARD_MAX_AGE_MS: Readonly<Partial<Record<string, number>>> = {
  // HN (6h), reddit (12h) and regional publishers (12h) share the NEWS type — gate on the widest.
  NEWS: 2 * (Math.max(FEED_TRIGGERS.NEWS_REDDIT_WINDOW_HOURS, FEED_TRIGGERS.NEWS_PUBLISHER_WINDOW_HOURS) * H + FEED_STALENESS_BUDGET_MS),
