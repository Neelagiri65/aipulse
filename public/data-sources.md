...
Alongside each tool's **declared** status page above, gawk.dev runs an **active reachability probe** — it hits the tool's real service endpoint and records whether it answered, from a single probe location. Shown symmetrically with the declared status, never merged into it; a disagreement (page says operational, probe unreachable) is surfaced, not resolved. The signal is "reachable", never "up/healthy" — a response proves the service answered, not that the backend is well. A single failed probe never asserts an outage (hysteresis). Latency is one-location round-trip (mostly geography). Endpoints (some undocumented; failure degrades to "pending", never fabricated): api.anthropic.com, api.openai.com, api.github.com, server.codeium.com, api2.cursor.sh.

### Status pages — platform infrastructure (4)
These four track the services gawk.dev itself runs on. Surfaced operator-side only (`/admin`); the public Tool Health card grid stays AI-focused.
- Vercel Status — host
...
### Machine summaries (1)
- NVIDIA NIM (Llama 4 Maverick) — a labelled, machine-written summary of the article a Hacker News link points to, only when the source published no text of its own. Always shown as machine-written, with the model named; never presented as the source's words.
...
gawk.dev re-broadcasts a subset of its public, source-cited cards into operator-facing channels. Every embed quotes the upstream status page verbatim — no editorial layer, no inferred severity.
