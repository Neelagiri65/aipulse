/**
 * Narration-driven walkthrough recorder.
 *
 * Reads narration-segments-youtube.json and navigates gawk.dev
 * to show the matching panel/view for each segment, timed to
 * match the audio duration.
 *
 * Fixes from v1:
 *   - No white screen: waits for full render + injects black bg
 *   - Proper framing: hides cookie banner, fills viewport
 *   - Clean overlays: styled lower-thirds injected via DOM (not ffmpeg drawtext)
 *
 * Output: out/walkthrough.webm
 * Usage: npx tsx scripts/video/record-narrated.ts
 */

import { chromium } from "@playwright/test";
import { mkdirSync, readFileSync, existsSync, renameSync } from "fs";
import { resolve } from "path";

const BASE_URL = process.env.GAWK_BASE_URL || "https://gawk.dev";
const ROOT = process.cwd();
const OUT_DIR = resolve(ROOT, "out");
const SEGMENTS_FILE = resolve(ROOT, "data/narration-segments-youtube.json");
...
async function hidePageChrome(page: import("@playwright/test").Page) {
  // Dismiss cookie consent by clicking "Reject all" (fastest way to remove it)
  try {
  ...
  await page.evaluate(() => {
    // Force-hide any remaining consent/toast overlays
    ...
    // Hide the privacy section at the bottom
...
async function navigateToPanel(page: import("@playwright/test").Page, panelId: string): Promise<boolean> {
  if (panelId === "globe") {
    // Click the map area to return to globe view
    const clicked = await page.evaluate(() => {
      ...
      // Fallback: click the map container
...
async function main() {
  if (!existsSync(SEGMENTS_FILE)) {
    console.error(`Missing: ${SEGMENTS_FILE}\nRun: npx tsx scripts/video/generate-narration.ts`);
    process.exit(1);
  }

  const segments: NarrationSegment[] = JSON.parse(readFileSync(SEGMENTS_FILE, "utf-8"));
  const totalDuration = segments.reduce((s, seg) => s + seg.durationSec + 0.8, 0);
  console.log(`Loaded ${segments.length} segments (${totalDuration.toFixed(0)}s total)\n`);
  ...
  // Dark background + styles immediately (before any navigation)
  await page.evaluate(() => {
  ...
  // Show branded intro overlay on the blank page — this is what the viewer sees
  // while gawk.dev loads behind it
  ...
  // Hold intro card briefly — more time accrues during page load behind it
  await page.waitForTimeout(1500);

  // Navigate to gawk.dev — the intro overlay persists because goto replaces the page,
  // so we need a different approach: use an iframe or navigate and re-inject.
  // Instead: open gawk.dev, immediately re-inject the intro overlay on top.
  console.log("Loading gawk.dev behind intro...");
  await page.goto(BASE_URL, { waitUntil: "domcontentloaded", timeout: 30000 });

  // Re-inject intro overlay immediately after navigation starts (covers loading state)
  await page.addStyleTag({ content: OVERLAY_CSS });
  await page.evaluate((date) => {
    document.documentElement.style.background = "#06080a";
    document.body.style.background = "#06080a";
  ...
  // Wait for full page render behind the overlay
  ...
  if (mapReady) {
    console.log("Pre-warming map tiles...");
    await page.evaluate(() => {
  ...
  // Fade out the intro overlay to reveal the loaded page
  ...
  console.log("Starting narration-driven recording...\n");

  let lastScene = "";

  for (let i = 0; i < segments.length; i++) {
    const seg = segments[i];
    const holdMs = Math.max(seg.durationSec * 1000, 2000);
    ...
    // Show lower-third overlay (except for intro/outro — those use badge only)
    ...
    // Scroll panel content for visual movement during longer segments
    if (seg.durationSec > 15 && seg.scene !== "globe") {
      try {
        await page.evaluate(() => {
  ...
  if (videoPath) {
    ...
    console.log("\nRecording complete — check out/ directory.");
  }
}
